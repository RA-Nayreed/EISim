...
/**
 * Implements a command-line argument parser. Options and corresponding values
 * are parsed
 * from a String array that contains all the command-line arguments given to the
 * program.
 * The parsed option values are saved into the {@link EisimSimulationParameters}
 * class,
...
public class ArgumentParser {
        ...
        public boolean parseArguments(String[] args) {
                try {
                        // Check whether help was requested, must be done separately as this.options has
                        // required options
                        ...
                        // In case that for some reason user specified the required options and also
                        // used help option
        ...
        /**
         * Creates a collection of Option objects that describe the possible options for
         * a command line.
         */
        protected void getOptions() {
                ...
                options.addOption(
                                Option.builder("S")
                                                .longOpt("stackelberg")
                                                .hasArg(false)
                                                .desc("Use the Myopic Stackelberg pricing agent instead of DDPG.")
                                                .build());
                options.addOption(
                                Option.builder("H")
                                                .longOpt("hetero")
                                                .hasArg(false)
                                                .desc("Use heterogeneous server configuration (capacity-differentiated costs).")
                                                .build());
                options.addOption(
                                Option.builder("P")
                                                .longOpt("base-properties")
                                                .hasArg()
                                                .argName("base_properties_file")
                                                .desc("Path to the common simulation_parameters.properties file.")
                                                .build());
                options.addOption(
                                Option.builder("topo")
                                                .longOpt("topology")
                                                .hasArg()
                                                .argName("topology_algorithm")
                                                .desc("The orchestration algorithm to use (C=CENTRALIZED, H=HYBRID, D=DECENTRALIZED).")
                                                .build());
        }

        /**
         * Saves the parsed option values into the {@link EisimSimulationParameters}
         * class.
         * 
         * @param cmd CommandLine object which represents the list of arguments parsed
         *            based on
         *            an Options object.
         */
        protected void setParameters(CommandLine cmd) {
                ...
                if (cmd.hasOption("random-steps")) {
                        EisimSimulationParameters.randomDecisionSteps = Math.max(0,
                                        Integer.parseInt(cmd.getOptionValue("random-steps")));
                }

                if (cmd.hasOption("replay-size")) {
                        EisimSimulationParameters.replayBufferSize = Math.max(0,
                                        Integer.parseInt(cmd.getOptionValue("replay-size")));
                }

                if (cmd.hasOption("batch-size")) {
                        EisimSimulationParameters.batchSize = Math.max(0,
                                        Integer.parseInt(cmd.getOptionValue("batch-size")));
                }

                if (cmd.hasOption("discount-factor")) {
                        EisimSimulationParameters.discountFactor = Math.max(0f,
                                        Math.min(1f, Float.parseFloat(cmd.getOptionValue("discount-factor"))));
                }

                if (cmd.hasOption("actorlr")) {
                        EisimSimulationParameters.learningRateActor = Math.max(0f,
                                        Float.parseFloat(cmd.getOptionValue("actorlr")));
                }

                if (cmd.hasOption("criticlr")) {
                        EisimSimulationParameters.learningRateCritic = Math.max(0f,
                                        Float.parseFloat(cmd.getOptionValue("criticlr")));
                }
                ...
                if (cmd.hasOption("updates")) {
                        EisimSimulationParameters.modelUpdates = Math.max(1,
                                        Integer.parseInt(cmd.getOptionValue("updates")));
                }

                if (cmd.hasOption("noisesd")) {
                        EisimSimulationParameters.noiseSD = Math.max(0f,
                                        Float.parseFloat(cmd.getOptionValue("noisesd")));
                }

                if (cmd.hasOption("noise-decay")) {
                        EisimSimulationParameters.noiseDecay = Math.max(0f,
                                        Float.parseFloat(cmd.getOptionValue("noise-decay")));
                }

                EisimSimulationParameters.heterogeneousConfig = cmd.hasOption("hetero");

                if (cmd.hasOption("base-properties")) {
                        EisimSimulationParameters.basePropertiesFile = cmd.getOptionValue("base-properties");
                }

                if (cmd.hasOption("topology")) {
                        EisimSimulationParameters.topology = cmd.getOptionValue("topology");
                }
        }
