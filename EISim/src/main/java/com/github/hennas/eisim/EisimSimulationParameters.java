...
/**
 * Holds the EISim simulation parameters. These specify where the simulation
 * setting files are located,
 * where to save simulation results and agent states, whether to seed the
 * simulation and with what seed,
...
public class EisimSimulationParameters {
	 ...
	 * 
	 * For each simulation scenario, a subfolder is created into this directory.
	 * Each edge server
	 * agent in the scenario creates its own subfolder into this scenario folder.
	 * The name of the
	 * subfolder is the same as the agent's name (the name is specified in
	...
	/**
	 * A seed for the seed generator. The seed generator generates the seeds for
	 * every random
	...
	/**
	 * Whether to use the Myopic Stackelberg pricing agent instead of DDPG.
	 */
	public static boolean useStackelberg = false;

	/**
	 * Whether the server configuration is heterogeneous.
	 */
	public static boolean heterogeneousConfig = false;

	/**
	 * Determines how many times at the beginning of a simulation each pricing agent
	 * decides the
	 * price randomly.
	 */
