...
/**
 * Extends the {@link DefaultComputingNode} class by adding in clustering and
 * pricing capabilities
 * for the edge servers.
 * <p>
 * The edge server clusters are determined offline. Cluster information must be
 * provided in the edge
 * server setting file (edge_datacenters.xml).
 * The implementation also assumes that all the edge servers have the same
 * specification, that is,
 * edge_datacenters.xml file should have the same MIPS, cores, RAM and storage,
 * as well as max and idle
 ...
 * At the beginning of each slot, an agent observes the current environment
 * state and makes a price
 * decision that determines the price for task execution at the cluster for the
 * duration of the slot.
 * During the slot, {@link EisimOrchestrator} records the tasks that are
 * offloaded to the cluster under
 * the current price. At the end of the slot, the agent calculates and logs the
 * profit it gained with
 * its price decision. The recorded information about the offloaded tasks is
 * used to calculate the total
 * revenue and the energy costs of task execution.
 * The profit (revenue - energy costs) is scaled to get the reward that the
 * agent gets for its price
 * decision. For the new slot, the agent observes the new state of the
 * environment and makes a price
 * decision.
 * <p>
 * If the simulation is run in the training mode as specified by
 * {@link EisimSimulationParameters#train},
 * at the end of a slot each agent records its experience in the form of a tuple
 * (state, action, reward, newState) into an experience replay and updates its
 * models with a minibatch
...
public class EisimComputingNode extends DefaultComputingNode {
	...
	// The following are only used for EDGE_DATACENTER type nodes that are also
	// cluster heads
	...
	protected int pricingSteps = 0; // Records how many pricing decisions have been made since the beginning of the
									// simulation
	...
	protected float capacityScale; // Used to scale the normalized capacity state variable
	protected double clusterTotalMips; // Total MIPS capacity of all servers in the cluster
	protected float normalizedCapacity; // Pre-computed normalized capacity for the state (constant per cluster)
	...
	protected double queueDelayEstimate; // An estimate of a queue time for one edge server (needed in centralized
											// control topology)
	protected INDArray previousState; // Records the previous state, which is needed during training at the beginning
										// of a new slot
										// to save the experience tuple (state, action, reward, newState)
	// Updating the following total counts is the responsibility of the
	// EisimOrchestrator
	protected int totalTasksArrivedInSlot = 0; // Needed when observing the new state at the beginning of a slot (see
												// getNewState())
	protected int totalMIsInSlot = 0; // Needed when calculating the profit for a slot (see getProfit())

	protected boolean isStateSaved = false; // Used to prevent double saving by shutdown hook and normal ending

	static {
		// Set your own custom pricing agent class here
		setCustomPricingAgentClass(DdpgPricingAgent.class);
		// Another option for setting custom pricing agent class is to extend this class
		// and
		// set the custom class in the static block of the child class (no need to
		// modify this)
	}
	 ...
	 * 
	 * @param simulationManager The simulation manager that links between the
	 *                          different modules.
	 */
	 ...
	 * @param simulationManager The simulation manager that links between the
	 *                          different modules
	 * @param mipsPerCore       The processing capacity of one CPU core in Million
	...
	/**
	 * Initialize other types of nodes (cloud data centers and edge devices, edge
	 * servers
	...
	/**
	 * Allows to use a custom pricing agent class in the simulation. The class must
	 * extend
	 ...
	 * When true, the state space is extended to 3 dimensions (adding normalized
	 * server capacity).
	...
	public void startInternal() {
		...
		if (!this.isAP()) {
			super.startInternal(); // Starts the UPDATE_STATUS event (for static energy consumption measurement and
									// mobility updates),
									// generates the mobility path

			if (this.getType() == SimulationParameters.TYPES.EDGE_DATACENTER) {
				// Updating the cluster member list
				this.simulationManager.getSimulationLogger()
						.deepLog("Finding cluster members for edge server " + this.getName());
				findEdgeClusterMembers();

				/*
				 * If the orchestration algorithm is CENTRALIZED, each edge server will save
				 * their queue delay estimate
				 * at the beginning of every price slot. This information is used by the central
				 * orchestrator when it
				 * allocates tasks. The idea here is to mimic the fact that central orchestrator
				 * will collect status
				 * information from the edge servers only at the beginning of each price slot,
				 * and then use this
				...
				// If the server is a cluster head, it must initialize a pricing agent and start
				// the PRICE_UPDATE event
				if (this.isClusterHead()) {
					this.simulationManager.getSimulationLogger()
							.deepLog("Starting price updates for edge server " + this.getName());
					initializeAgent();
					scheduleNow(this, PRICE_UPDATE);
				}
			}

			// If the node is an end device, it must generate its individual weights for
			// making offloading decisions
			if (this.getType() == SimulationParameters.TYPES.EDGE_DEVICE) {
				this.simulationManager.getSimulationLogger()
						.deepLog("Generating device weights for edge device " + this.getId());
	...
	public void processEvent(Event e) {
		switch (e.getTag()) {
			case PRICE_UPDATE:
				updatePrice();
				// No need to do price updates anymore if we are just waiting for the tasks to
				// finish (simClock > simDuration)
	...
	public void onSimulationEnd() {
		// Save the price log, models and experience replay for the edge servers that
		// are also cluster heads
	 ...
	 * @return boolean: True if the cluster member list contained the given
	 *         computing node and it was removed.
	...
	/**
	 * Gets the current price for task execution in the cluster. Should be only used
	 * for cluster heads.
	 * 
	...
	/**
	 * Sets the current price for task execution in the cluster. Should be only used
	 * for cluster heads.
	 * 
	...
	/**
	 * Gets the current queue time estimate inside a cluster for the duration of the
	 * slot.
	 * Should be only used for cluster heads in decentralized and hybrid control
	 * topologies.
	...
	/**
	 * Sets the current queue time estimate inside a cluster for the duration of the
	 * slot.
	 * Should be only used for cluster heads in decentralized and hybrid control
	 * topologies.
	 * 
	...
	/**
	 * Gets the current queue delay estimate at the edge server for the duration of
	 * the slot.
	 * Should be only used for edge servers in centralized control topology.
	 * (Central orchestator uses this information when making task allocation
	 * decisions)
	 * 
	...
	/**
	 * Sets the current queue delay estimate at the edge server for the duration of
	 * the slot.
	 * Should be only used for edge servers in centralized control topology.
	 * 
	...
	/**
	 * Adds information about the tasks that are offloaded to a cluster during a
	 * slot.
	...
	/**
	 * Finds the cluster members of an edge server node and adds them to the cluster
	 * member list.
	 * Also records the size of the cluster.
	 * The created list does not include the edge server itself, but the recorded
	 * cluster size does.
	 */
	protected void findEdgeClusterMembers() {
		// Looking for cluster members only if this node has an assigned cluster
		// (non-negative cluster integer)
		if (this.getCluster() > -1) {
			// List of all edge nodes (includes APs)
			List<ComputingNode> edgeNodes = this.simulationManager.getDataCentersManager().getComputingNodesGenerator()
					.getEdgeOnlyList();
	...
	/**
	 * Generates the importance weights for edge devices. These are used in the
	 * orchestration of the
	 * tasks.
	 * <p>
	 * Each edge device needs three weights: the first weight corresponds to the
	 ...
	 * Hence, the possible weight values lie on a triangle formed by the points
	 * (1,0,0), (0,1,0) and
	 * (0,0,1) inside a unit cube.
	 * The coordinates of each point on the triangle correspond to the weight
	 * values.
	 * This method generates the weight values by randomly sampling a point from the
	 * triangle.
	 */
	protected void generateDeviceWeights() {
		this.deviceWeights = new ArrayList<>(3);

		double[] randomValues = { this.random.nextDouble(), this.random.nextDouble() }; // Two random values both
		...
		double weight3 = randomValues[1] - randomValues[0]; // The final weight is the difference between the larger and
	...
	/**
	 * Initializes the {@link PricingAgent} instance and the {@link PriceLogger}
	 * instance for
	 * EDGE_DATACENTER type nodes that are also cluster heads.
	 * Also the scales for the reward and state variables are initialized according
	 * to the used
	 * orchestration algorithm.
	 */
	protected void initializeAgent() {
		this.clusterTotalMips = this.getTotalMipsCapacity();
		for (EisimComputingNode member : this.getClusterMembers()) {
			this.clusterTotalMips += member.getTotalMipsCapacity();
		}

		// If heterogeneous, compute the normalized capacity for the state
		if (heterogeneousConfig) {
			// Compute scenario average MIPS per cluster by summing all edge server MIPS
			// and dividing by the number of cluster heads (i.e., clusters)
			List<ComputingNode> edgeNodes = this.simulationManager.getDataCentersManager()
					.getComputingNodesGenerator().getEdgeOnlyList();
			double totalScenarioMips = 0;
			int numClusters = 0;
			for (ComputingNode node : edgeNodes) {
				EisimComputingNode eisimNode = (EisimComputingNode) node;
				if (!eisimNode.isAP() && eisimNode.getCluster() > -1) {
					totalScenarioMips += eisimNode.getTotalMipsCapacity();
				}
				if (eisimNode.isClusterHead()) {
					numClusters++;
				}
			}
			double scenarioAvgMipsPerCluster = numClusters > 0 ? totalScenarioMips / numClusters : totalScenarioMips;
			// Normalized capacity: ratio of this cluster's MIPS to the scenario average
			this.normalizedCapacity = scenarioAvgMipsPerCluster > 0
					? (float) (this.clusterTotalMips / scenarioAvgMipsPerCluster)
					: 1.0f;
		}
		...
		// graceful shutdown hook to save states in case of premature termination
		Runtime.getRuntime().addShutdownHook(new Thread(() -> {
			if (!this.isStateSaved) {
				this.isStateSaved = true;
				System.out.println("SHUTDOWN HOOK: Saving price log and agent state for edge server " + this.getName()
						+ " before exiting...");
				try {
					this.priceLog.saveLog();
					this.agent.saveAgentState();
	 ...
	 * <p>
	 * This method is called at the beginning of every new slot when handling the
	 * {@code PRICE_UPDATE}
	...
	protected void updatePrice() {
		...
		this.priceLog.addLine(this.simulationManager.getSimulation().clock(), this.getPrice(), profit,
				this.previousState);
		...
		// If the simulation is run in training mode, the experience tuple is saved and
		// the models updated
		...
		float newPrice = this.pricingSteps >= EisimSimulationParameters.randomDecisionSteps // If the number of random
																							// pricing steps
	...
	/**
	 * Gets the new state at the beginning of a slot. This state is used to make the
	 * next price decision.
	 * 
	...
	protected INDArray getNewState() {
		...
		double avgTaskQueueLength = (double) (totalQueueLength + this.getTasksQueue().size()) / this.clusterSize; // Average
																													// task
																													// queue
																													// length
																													// in
																													// the
																													// cluster
		double avgArrivalRate = (double) this.totalTasksArrivedInSlot / PRICE_UPDATE_INTERVAL; // Average arrival rate
																								// of tasks
		state[0][0] = Math.min(10.0f, (float) (this.queueScale * avgTaskQueueLength));
		state[0][1] = Math.min(10.0f, (float) (this.arrivalRateScale * avgArrivalRate));
		if (heterogeneousConfig) {
			state[0][2] = Math.min(10.0f, this.capacityScale * this.normalizedCapacity); // Normalized cluster capacity
		}
	 ...
	 * <p>
	 * The profit of the cluster = total revenue - varying energy cost - fixed
	 * energy cost.
	 * 
	 * @return double: Scalar profit value
	 */
	protected double getProfit() {
		double revenue = this.getPrice() * this.totalMIsInSlot; // price per MI * total MIs offloaded to this cluster
																// during slot

		// Varying energy cost: uses actual cluster total MIPS (handles heterogeneous
		// servers)
		// cost per J * (avg power difference) * processing time
		double totalMaxConsumption = this.getEnergyModel().getMaxActiveConsumption();
		double totalIdleConsumption = this.getEnergyModel().getIdleConsumption();
		for (EisimComputingNode member : this.getClusterMembers()) {
			totalMaxConsumption += member.getEnergyModel().getMaxActiveConsumption();
			totalIdleConsumption += member.getEnergyModel().getIdleConsumption();
		}
		double varyingEnergyCost = energyCostCoefficient * (totalMaxConsumption - totalIdleConsumption)
				* this.totalMIsInSlot / this.clusterTotalMips;
		// Fixed energy cost: cost per J * total idle energy consumption in the cluster
		// during the price slot
		double fixedEnergyCost = energyCostCoefficient * totalIdleConsumption * PRICE_UPDATE_INTERVAL;
		double profit = revenue - varyingEnergyCost - fixedEnergyCost; // The profit gained as a result of the price
																		// decision
		return profit;
	}

	/**
	 * Calculates a rough estimate of the queuing time inside a cluster for the new
	 * slot.
	 * Edge devices use this information in their offloading decisions in
	 * decentralized and hybrid
	 * control topologies.
	 * <p>
	 * The queue time estimate is the total number of MIs summed over all task
	 * queues and tasks in the
	 * cluster divided by the total processing capacity (MIPS) of the cluster.
	 * <p>
	 * This method is called only for edge servers that are also cluster heads
	 * during the handling of
	 * {@code PRICE_UPDATE} event.
	 * 
	 * @return double: A rough estimate of the queuing time at the cluster during
	 *         the next slot
	 */
	protected double calculateQueueTimeEstimateForSlot() {
		...
		double estimate = totalMIs / this.clusterTotalMips; // uses actual total cluster MIPS (handles heterogeneous
															// servers)
		return estimate;
	}

	/**
	 * Calculates a rough estimate of the queuing time at an edge server for the new
	 * slot.
	 * The central orchestrator uses this information when allocating tasks in
	 * centralized control
	 * topology.
	 * <p>
	 * This method is called for edge servers at the beginning of every new price
	 * slot when handling the
	 * {@code RECORD_QUEUE_DELAY_ESTIMATE} event.
	 * 
	 * @return double: A rough estimate of the queuing delay at the edge server
